import { SelectEncargos } from "@/components/SelectEncargos";
import { Search, ChevronDown, X } from "lucide-react";
import { useOrder } from "../hooks/useOrder";
import type { ColumnDef } from "@tanstack/react-table";
import type { Encargo } from "@/types/Product";
import { OrderTable } from "../components/OrderTable";
import { OrderSelectRowContextProvider } from "../context/OrderSelectRowContext";
...
export const GestionEncargos = ({}: Props) => {
			...
			cell: ({row})=> {
				...
				return (
					<div className="flex justify-center items center">
						<div className={`text-sm rounded-3xl w-[125px] h-[27px] font-bold  flex justify-center items-center ${status==='completado'?'bg-[#2C4CA4] text-blue-100 ':'bg-[#7e6f1e]'}`}>
							<span>{status.replace(/^./,letra => letra.toUpperCase())}</span>
						</div>
					</div>
				)
			}
		},
		{
			...
			cell: ({ row }) => (
				<div className="flex gap-3 justify-center items-center">
					...
					>
						<ChevronDown className={`${rowExpand.includes(row.original.id)?'rotate-180':''} w-5 h-5 transition-all ease-in-out duration-300 `} />
					</button>
	...
	const { table } = useCoustomTable({ data: encargos, columns: columOrder,selctedRow:false });
	...
	return (
		<div className="h-full w-full py-4 px-10 flex flex-col gap-4">
			<div className="flex justify-center items-center">
				<h1 className="text-3xl text-gray-200 font-bold">Gestión de encargos</h1>
			</div>
