...
export const EditProduct = ({ product, onClouse }: Props) => {
  ...
  return (
    <div className="fixed inset-0 z-20 flex items-center justify-center w-screen h-screen  backdrop-blur-sm">
      {/* Modal */}
      <div className="relative bg-gray-800 rounded-xl shadow-2xl w-[90%] max-w-3xl h-[90vh] flex flex-col sm:flex-row overflow-hidden animate-scaleIn">
        
        {/* Botón de cerrar - Esquina superior derecha del modal */}
        <button
          onClick={onClouse}
          className="absolute top-2 right-2 bg-gray-800 text-gray-100/70 cursor-pointer text-2xl w-8 h-8 flex items-center justify-center rounded-full hover:bg-opacity-90 transition z-20"
