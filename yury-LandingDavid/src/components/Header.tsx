...
const Header: React.FC = () => {
  ...
  return (
    <>
      {/* Barra superior */}
      <header className="flex text-white justify-between items-center w-full px-2  shadow-md py-3 bg-gray-900 fixed z-10 min-h-[5rem]">
          ...
          <button className="hover:bg-white/5 text-white border-none rounded-lg sm:px-3 py-2 cursor-pointer transition-colors font-medium text-xs md:text-sm  sm:flex items-center">
            <Link className="flex gap-2" to={"favorite"}>
              <div className="flex relative">
                <Heart
                  fill={favoritos > 0 ? "#fff" : "#101828"}
                  className="h-5 w-5 "
                />
                {favoritos > 0 && (
                  <span className="text-xs self-start absolute text-gray-900 top-[2px] font-bold left-0 bottom-0 right-0">
                    {favoritos}
                  </span>
                )}
              </div>{" "}
      ...
      <div
        className={`fixed top-0 left-0 h-full w-80 bg-gray-900 text-white z-50 transform transition-transform duration-300 overflow-y-auto scroll-container ${
