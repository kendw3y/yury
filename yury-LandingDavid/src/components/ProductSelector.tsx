...
const ProductSelector: React.FC<ProductSelectorProps> = ({
  ...
  const personalizations = [
    "TODOS",
    ...Array.from(new Set(products.map((p) => p.personalizacion))),
  ];

  const filteredProducts = products.filter((product) => {
    const personalizationMatch =
      selectedPersonalization === "TODOS" ||
      product.personalizacion === selectedPersonalization;
    return personalizationMatch;
  });

  return (
    <div className="rounded-lg bg-gray-900 shadow p-6 mb-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          <label
            className="block text-sm font-medium mb-3"
            style={{ color: "#ffffff", fontFamily: "Nunito", fontWeight: 600 }}
          >
          ...
          <div className="relative">
            ...
            >
              {personalizations.map((personalization) => (
                <option
                  ...
                  style={{ backgroundColor: "#111420", color: "#ffffff" }}
                >
                  {personalization}
                </option>
              ))}
            ...
            <ChevronDown
              className="absolute right-3 top-3.5 h-5 w-5 pointer-events-none"
              style={{ color: "#335BC6" }}
            />
          </div>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              >
                Áreas: {selectedProduct.areas.join(", ")} • Colores máx:{" "}
                {selectedProduct.coloresMax}
              </p>
            </div>
